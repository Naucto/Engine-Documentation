...
![The CODE tab](img/code.png "The CODE tab of a new game after one ▶: the tab strip and the editor in the middle, the running screen at the top right with the transport under it, and the Console showing the game's greeting.")
...
![Signature help while typing a call](img/code-signature.png "The signature help of gfx.draw_sprite: each argument with its type and one line on what it is for, and the one being written, x, picked out in gold.")
...
When the game throws, the failing line is **underlined and tinted**, with a mark in the gutter and the message on hover. The status bar shows `1 error`, and the Console tab of the column carries a badge with the count. Fix the line: with Auto on, the game reruns by itself once the line is fixed, and there is nothing to press.

![A runtime error in the editor](img/code-error.png "A runtime error on line 3 of main: the line is tinted, with a mark in its gutter; the status bar reads 1 ERROR; the Console tab carries the badge and shows the red line that names the tab and the line, then --- HALTED ---.")

## Tabs

![The tab strip](img/code-tabs.png "The tab strip of a game with three tabs, 1 main, 2 player and 3 enemies, numbered in the order they run; the + and the magnifier are at the right end.")

A game holds as many tabs as it needs. Every launch runs each tab once, **in the order of the strip**, and only then calls `_init`. Drag a tab to change that order; the number beside its name is its rank. There is no way to ask for a tab from code: it has already run by the time the tabs after it start, so a tab that needs something at the moment it runs, rather than later from inside `_init`, wants the tab that provides it earlier in the strip.
...
![The console column](img/code-console.png "The console column before the first run: an idle screen with the pointer, the transport under it with Play, Restart, Step, the Auto switch, the P1 and P2 device marks, the Gamepads button and Fullscreen, and the Console saying the machine has nothing to say yet.")

The column at the right of CODE holds the **running game** at its own 320 × 180, scaled to the column. Opening the editor does not launch it; the transport under the screen does: Play or Pause, Restart, Step (one `_update` and one `_draw`, pausing a running game first), then the Auto switch, the device marks and Fullscreen. Click the screen to give it the keyboard.

The **device marks**, `P1` and `P2`, are the player seats: each carries a keyboard or a gamepad icon for the device that drives it, dimmed while nobody sits there. The gamepad button beside them opens the **Gamepads** popover, which lists every connected pad and lets you assign it to a seat, P1 to P4; with no pad it says so, and pressing any button on one makes it appear.

The **Auto** switch, on by default, reruns the game 400 ms after an edit while it runs, or after an error once you fix it, so the screen shows what you just typed; a paused game is reloaded with the new code and stays paused. Turn it off while a change spans several tabs, or while a netplay session is running, since a rerun ends it.
...
**Perf** is seven readings: `FPS`, `CPU` as a percentage of the step's budget, `FRAME`, `STATE` of the runtime, `PLAYERS` as `n / max` in a netplay session and `—` outside one, `EDITORS`, the number of people editing the project right now, and `SYNC` with the server.
...
![The reference beside the console](img/code-reference-split.png "In a window 1602 px wide or more, the REFERENCE opens between the code and the console column: its search box and the tree of the documentation, with the game screen and the Console still at the right, here before the first run.")
