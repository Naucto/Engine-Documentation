...
![The MAP tab](img/map.png "The MAP tab: the map in the middle, the tools above it, and the TILE PICKER and WHOLE MAP panels at the right.")
...
**Flags** tints every tile by the first flag set on its sprite, in the colour that bit has in the FLAGS panel of ART. It is the quickest way to see a level's collision: turn it on, and every solid tile lights up, every tile you forgot stays dark.
...
![The TILE PICKER panel](img/map-brush.png "The TILE PICKER: drag a rectangle of cells to make a brush of several tiles.")

The TILE PICKER at the top of the right panel shows a sheet. Click a cell to make it the brush, or **drag a rectangle** of up to 8 × 8 cells to stamp a block of tiles at once: a whole tree, a platform end to end. The Tilesets strip above the picker chooses which sheet, since a map can take its tiles from any of them.
...
Every `map.*` function takes the map it works on as its **last argument**, counted from `1` in the order of the strip. Leave it out and the first map is used, so a game with one map never names it.
...
The engine has no collision helpers. What it has is the map and the flags: mark the solid sprites with bit 0 in ART, then ask the map what is under a point. Tile coordinates are pixel coordinates divided by 8.
